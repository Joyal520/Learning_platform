        ...
        form.addEventListener('submit', async (e) => {
            ...
            try {
                ...
                if (contentMode === 'text') {
                    ...
                    fileToUpload = this._imageCompressedBlob || this._imageFile;
                    if (fileToUpload) {
                        finalFileSize = fileToUpload.size;
                        finalFileType = fileToUpload.type || 'image/webp';
                    }
                }
                ...
                if (category === 'images') {
                    ...
                    const submissionData = {
                        ...
                        compressed_size: finalFileSize,
                        image_width: meta.width || 0,
                        image_height: meta.height || 0,
                        mime_type: this._imageFile?.type || finalFileType,
                        status: 'approved', // Auto-approving images
                    ...
                    const { error } = await API.uploadImagePost(
                        submissionData,
                        fileToUpload,
                        this._imageThumbnailBlob || this._imageFile // Fallback thumbnail
                    );
                    ...
                    if (error) {
                        ...
                        this._resetImageSelection();
                        setTimeout(() => window.location.hash = '#explore', 1500);
                    }
    ...
    async _handleImageSelect(file) {
        ...
        if (originalSizeKB > 100) {
            ...
            try {
                ...
                this._imagePreviewUrl = URL.createObjectURL(result.blob);
                if (previewImg) {
                    previewImg.src = this._imagePreviewUrl;
                    previewImg.style.filter = 'none';
                }

                const savedPercent = Math.round((1 - result.blob.size / file.size) * 100);
                if (compressionStatus) {
                    compressionStatus.innerHTML = `<span class="compression-done">✅ Compressed: ${(result.blob.size / 1024).toFixed(0)} KB (saved ${savedPercent}%)</span>`;
                }

                if (previewInfo) {
                    previewInfo.innerHTML = `<span class="preview-filename">${file.name}</span><span class="preview-size">${(result.blob.size / 1024).toFixed(0)} KB <span style="color: #22c55e; font-size: 0.8em;">(was ${originalSizeKB.toFixed(0)} KB)</span></span>`;
                }
            } catch (err) {
                console.error('Image compression failed:', err);
                UI.showToast('Compression failed. Using original.', 'warning');
