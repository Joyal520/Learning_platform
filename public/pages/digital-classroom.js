...
const DIGITAL_CLASSROOM_ASSET_VERSION = "student-exam-redesign-v4";
const PAGE_FILES = {
    ...
    "student-exams": "student-exams.html",
    "student-exam": "student-exam.html",
...
function routeToStandaloneHref(href) {
    ...
    if (file === "exam.html" && classroomId) return `#classroom/exam/${encodeURIComponent(classroomId)}`;
    ...
    if (file === "student-exams.html" && classroomId) return `#classroom/student-exams/${encodeURIComponent(classroomId)}`;
    if (file === "student-exam.html" && classroomId) {
        const examId = url.searchParams.get("examId") || "";
        return `#classroom/student-exam/${encodeURIComponent(classroomId)}?examId=${encodeURIComponent(examId)}`;
    }
...
export const DigitalClassroomPage = {
    ...
    async init(pageKey = "dashboard", params = {}) {
        ...
        if (!["student", "student-exams", "student-exam", "join", "my-classes"].includes(pageKey)) {
        ...
        if (pageKey === "exam") {
            await window.ClassroomUI.renderExam2(classroomId);
        }
        ...
        if (pageKey === "student-exams") {
            await window.ClassroomUI.renderStudentExams(classroomId);
        }

        if (pageKey === "student-exam") {
            await window.ClassroomUI.renderStudentExam(classroomId);
        }
