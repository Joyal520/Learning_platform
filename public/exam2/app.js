...
const examRouteParams = new URLSearchParams(window.location.search);
const examContext = {
  source: examRouteParams.get("source") || "edtechra",
  classroomId: examRouteParams.get("classroomId") || examRouteParams.get("classId") || "",
  teacherId: examRouteParams.get("teacherId") || examRouteParams.get("teacher_id") || ""
};

function examApiUrl(action) {
  const [name, ...queryParts] = String(action).split("&");
  const query = queryParts.length ? `&${queryParts.join("&")}` : "";
  return `/api/exam-engine?action=${encodeURIComponent(name)}${query}`;
}

function getTokenFromLocalStorage() {
  // Supabase JS v2 stores session in localStorage under key "sb-<projectRef>-auth-token"
  // The project ref is derived from the Supabase URL
  try {
...
async function examAuthHeaders() {
  try {
    // Strategy 1: Ask parent iframe bridge (when embedded in Digital Classroom)
    const parentBridge = window.parent?.DigitalClassroomSupabase;
    let session = await parentBridge?.getSession?.();
    let token = session?.access_token;

    // Strategy 2: Own Supabase client in this frame's window
    if (!token) {
      const ownClient = window.DigitalClassroomSupabase || window.supabaseClient;
      session = await ownClient?.getSession?.();
      token = session?.access_token;
    }

    // Strategy 3: Read directly from shared localStorage (same-origin fallback)
    if (!token) {
      token = getTokenFromLocalStorage();
    }

    if (!token) {
      console.warn("[exam2] examAuthHeaders: No access token found via parent bridge, own client, or localStorage.");
    }
    return token ? { Authorization: `Bearer ${token}` } : {};
  } catch (error) {
    console.error("[exam2] examAuthHeaders error:", error);
    return {};
  }
}

async function examFetch(action, options = {}) {
  const headers = {
    ...(options.body ? { "Content-Type": "application/json" } : {}),
    ...(await examAuthHeaders()),
    ...(options.headers || {})
  };
  return fetch(examApiUrl(action), { ...options, headers });
}
...
const state = {
  ...
  publish: {
    classroom: examContext.classroomId ? `Classroom ${examContext.classroomId}` : "Current Edtechra Classroom",
    startDate: "",
    startTime: "",
    endDate: "",
    endTime: "",
...
if (app) {
  app.dataset.source = examContext.source;
  app.dataset.classroomId = examContext.classroomId;
}
...
function render() {
  ...
  app.innerHTML = `
    <div class="shell">
      <aside class="nav">
        <div class="brand"><img src="/exam2/assets/logo%20dark.png" alt="Edtechra Logo" class="brand-logo" /></div>
...
function renderTeacherExamList() {
  const exams = state.teacherExams.rows || [];
  return `
    <section class="section-head row" style="margin-top: 28px;">
      <div><h2>Classroom Exams</h2><p>Published and draft exams stored for this classroom.</p></div>
      <button class="secondary" data-action="load-teacher-exams" type="button">${state.teacherExams.loading ? "Loading..." : "Refresh"}</button>
    </section>
    ${state.teacherExams.error ? `<div class="lockbox">${escapeHtml(state.teacherExams.error)}</div>` : ""}
    ${!exams.length ? `
      <div class="empty" style="padding: 28px;">
        <h2>No classroom exams loaded</h2>
        <p>Publish an exam or refresh the list to view classroom exams and submissions.</p>
      </div>
    ` : `
      <div class="result-grid" style="grid-template-columns: repeat(auto-fit, minmax(240px, 1fr));">
        ${exams.map((exam) => `
          <article class="metric" style="text-align: left;">
            <strong style="font-size: 18px;">${escapeHtml(exam.title || "Untitled Exam")}</strong>
            <span>${escapeHtml(exam.runtime_status || exam.status || "draft")} · ${Number(exam.total_marks || 0)} marks</span>
            <span>${Number(exam.stats?.submissions || 0)} submissions · Avg ${Number(exam.stats?.averageScore || 0)}%</span>
            <button class="secondary" data-action="view-exam-results" data-id="${escapeHtml(exam.id)}" type="button" style="margin-top: 12px;">View Results</button>
          </article>
        `).join("")}
      </div>
    `}
    ${state.teacherExams.selectedResults.length ? `
      <div class="review-card" style="margin-top: 18px;">
        <h3>Student Results</h3>
        <div style="overflow-x: auto;">
          <table style="width: 100%; border-collapse: collapse;">
            <thead><tr><th align="left">Student</th><th align="left">Attempt</th><th align="left">Score</th><th align="left">Grade</th><th align="left">Status</th><th align="left">Report</th></tr></thead>
            <tbody>
              ${state.teacherExams.selectedResults.map((result) => `
                <tr>
                  <td>${escapeHtml(result.student_id || "Student")}</td>
                  <td>${Number(result.attempt_number || 1)}</td>
                  <td>${Number(result.score || 0)}/${Number(result.max_score || 0)} (${Number(result.percentage || 0)}%)</td>
                  <td>${escapeHtml(result.grade || "")}</td>
                  <td>${escapeHtml(result.status || "")}</td>
                  <td><button class="secondary" data-action="download-student-report" data-id="${escapeHtml(result.id)}" type="button">PDF</button></td>
                </tr>
              `).join("")}
            </tbody>
          </table>
...
async function handleAction(event) {
  ...
  if (action === "load-teacher-exams") await loadTeacherExams();
  if (action === "view-exam-results") await loadTeacherExamResults(event.currentTarget.dataset.id);
  if (action === "download-student-report") await downloadStudentReport(event.currentTarget.dataset.id);
...
async function generateExam() {
  ...
  try {
    const response = await examFetch("generate-exam", {
      method: "POST",
      ...
      body: JSON.stringify({
        ...
        classroomId: examContext.classroomId,
        teacherId: examContext.teacherId,
        source: examContext.source,
...
async function saveExam() {
  const response = await examFetch("save-exam", {
    method: "POST",
    body: JSON.stringify({
      ...
      publishing: state.publish
    })
  });
  const data = await response.json();
  if (response.ok) await loadTeacherExams({ silent: true });
  toast(response.ok ? `Saved draft ${data.examId}.` : data.error);
}

async function publishExam() {
  const response = await examFetch("publish-exam", {
    method: "POST",
    body: JSON.stringify({
      exam: state.generatedExam,
      approved: state.approved,
  ...
  const data = await response.json();
  if (response.ok) {
    await loadTeacherExams({ silent: true });
    toast(`Published ${data.examId}. Status: ${data.status}.`);
  } else {
    toast(data.error);
  }
}

async function loadTeacherExams(options = {}) {
  if (!examContext.classroomId) {
    state.teacherExams.error = "Missing classroom ID for exam list.";
    return;
  }
  state.teacherExams.loading = true;
  state.teacherExams.error = "";
  if (!options.silent) render();
  try {
    const response = await examFetch(`list-exams&classroomId=${encodeURIComponent(examContext.classroomId)}`);
    const data = await response.json();
    if (!response.ok) throw new Error(data.error || "Could not load classroom exams.");
    state.teacherExams.rows = data.exams || [];
  } catch (error) {
    state.teacherExams.error = error.message || "Could not load classroom exams.";
  } finally {
    state.teacherExams.loading = false;
  }
}

async function loadTeacherExamResults(examId) {
  state.teacherExams.selectedExamId = examId;
  state.teacherExams.loading = true;
  render();
  try {
    const response = await examFetch(`get-exam-results&examId=${encodeURIComponent(examId)}`);
    const data = await response.json();
    if (!response.ok) throw new Error(data.error || "Could not load exam results.");
    state.teacherExams.selectedResults = data.results || [];
    state.teacherExams.error = "";
  } catch (error) {
    state.teacherExams.error = error.message || "Could not load exam results.";
  } finally {
    state.teacherExams.loading = false;
  }
}

async function downloadStudentReport(resultId) {
  const response = await examFetch(`generate-student-report-pdf&resultId=${encodeURIComponent(resultId)}`);
  const data = await response.json();
  if (!response.ok) {
    toast(data.error || "Could not generate report PDF.");
    return;
  }
  window.open(data.report_pdf_url, "_blank", "noopener");
  toast("Student report PDF generated.");
}
...
async function gradeDemo() {
  ...
  const response = await examFetch("grade-attempt", {
    method: "POST",
    body: JSON.stringify({ exam: state.generatedExam, answers, classroomId: examContext.classroomId, source: examContext.source })
  });
  const data = await response.json();
...
const mockClassResults = {
  exam_id: "EXAM_001",
  class_id: examContext.classroomId || "CLASS_A",
...
async function loadResults() {
  ...
  render();
  try {
    const response = await examFetch("score-analysis", {
      method: "POST",
